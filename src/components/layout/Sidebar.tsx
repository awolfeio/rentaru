...
export function Sidebar() {
  return (
    <aside className="fixed left-0 top-0 z-30 w-64 h-screen bg-sidebar text-sidebar-text border-r border-sidebar-hover flex flex-col pt-6 pb-4">
      ...
      <nav className="flex-1 overflow-y-auto px-4 space-y-6">
        {NAV_GROUPS.map((group, groupIndex) => (
          <div key={groupIndex}>
            <h3 className="text-xs font-semibold text-sidebar-text/50 uppercase tracking-wider mb-2 pl-2">
              {group.label}
            </h3>
                      ...
                      <>
                        <item.icon
                          className={cn(
                            ...
                            isActive ? "text-primary" : "text-sidebar-text group-hover:text-sidebar-textActive"
                          )}
                        />
                        {item.name}
                      </>
